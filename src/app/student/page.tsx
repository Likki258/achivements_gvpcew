...
export default function StudentDashboard() {
  ...
  const filteredAchievements = achievements.filter((a) => {
    return (
      (!filterType || a.achievementType === filterType) &&
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-purple-50 flex">
      ...
      <div className="w-72 bg-gradient-to-b from-purple-700 to-indigo-800 text-white p-4 shadow-lg">
        <div className="flex items-center mb-8 mt-4">
          ...
          <div>
            <h2 className="font-bold text-lg">{user?.displayName || 'Student'}</h2>
            <p className="text-purple-200 text-xs truncate">{user?.email || ''}</p>
          </div>
        </div>
        <nav>
          <button onClick={() => setActiveTab('submit')} className={`w-full flex items-center space-x-3 p-3 rounded-lg mb-2 transition-all ${activeTab === 'submit' ? 'bg-white text-purple-800 font-medium' : 'text-white hover:bg-purple-600'}`}>
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
            </svg>
            <span>Submit New Achievement</span>
          </button>
          <button onClick={() => setActiveTab('submissions')} className={`w-full flex items-center space-x-3 p-3 rounded-lg transition-all ${activeTab === 'submissions' ? 'bg-white text-purple-800 font-medium' : 'text-white hover:bg-purple-600'}`}>
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="bg-white p-8 rounded-xl shadow-lg border border-gray-100">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-gray-700 mb-2 font-medium">Achievement Type</label>
                  <select
                    className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gradient-to-r from-purple-50 to-indigo-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-purple-800 uppercase tracking-wider">Roll No</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredAchievements.map((a) => (
                    <tr key={a.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{a.studentName}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{a.rollNo}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{a.achievementType}</td>
